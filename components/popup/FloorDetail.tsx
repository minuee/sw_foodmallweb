...
const FloorDetail: React.FC<PopupFloorDetailProps> = ({ selectFloor,setSeq, setSelectFloor,setPopDetailOpen}) => {
    // 화면 가로모드 여부 확인
    ...
    return (
        <NoSsr>
            <Box ref={popDetailRef} sx={styles.outerWrapper}>
                ...
                <Box sx={{position:'absolute',bottom:'393px',left:0,height:'auto',width:'100%',alignItems:'flex-end'}}>
                    <Slider {...settings}>
                        <Image 
                            alt={"sub"}
                ...
                <Box sx={styles.popupDataWrap}>
                    <Box sx={{...styles.boxWrapper,marginBottom:"10px"}}>
                    ...
                    <Box ref={popDetailRef} sx={styles.moreWrapper}>
                        <Button variant="outlined" sx={styles.popupButton}>
                            <Link href={selectFloor?.selectData?.desktopUrl} style={{color:'#fff',textDecoration: "none" }} >
                                <Typography variant="sourceHanSans" sx={styles.popupSubTitle}>
                                    자세히 보기 +
                                </Typography>
                            </Link>
...
const styles = {
    ...
    outerWrapper : {
        display:'flex',position:'relative',flexDirection:'column',flex:1,justifyContent:'flex-end',width:'100%',height:'100vh',maxWidth:'430px',paddingBottom:'150px',backgroundColor:'transparent'
    },
    boxWrapper : {
    ...
    closeBoxWrap : {
          position:'absolute',right:'5px',bottom:'700px',width:'30px',height:'30px',borderRadius:'15px',backgroundColor:'#f7f7f7',display:'flex',justifyContent:'center',alignItems:'center', zIndex:10,cursor:'pointer'
    },
    nextArrow : {
