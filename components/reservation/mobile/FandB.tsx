...
import CustomCalendar from "components/reservation/mobile/CustomCalendar";
import SelectDining from "components/reservation/mobile/SelectDining";
...
export default function Reservation() {
  ...
  const onSelectDate = (date:any) => {
    ...
    if ( disableDays.find((x:any) => x === moment(date).format("YYYY-MM-DD")) ) {
        ...
        onDateChange(date);
        setInputs({...inputs,reservation_date: moment(date).format("YYYY-MM-DD")})
    }
  ...
  const renderLable = (index:number,step:any) => {
    if ( step.code == 'step1' ) {
        ...
        return (
            `${step.label} ${functions.isEmpty(inputs.reservation_date) ? "" : inputs.reservation_date+"("+(functions.convertDatetoWeekday(inputs.reservation_date))+")" }`
        )
  ...
  const renderNextButton = (index:number,step:string) => {
    if ( step == 'step1' ) {
        ...
        return (
            <Button
                disabled={functions.isEmpty(inputs.reservation_date)}
                variant="contained"
                onClick={handleNext}
                sx={{ mt: 1, mr: 1 }}
            >
